define(['lang', 'logger', 'event', 'timer', 'Promise'], function(lang, logger, event, timer, Promise){
  ...
  Engine.prototype = {
      ...
      this.loop = new timer.Loop({
        ...
        tick: function(timerData){
          try {
            ...
            console.warn("Exception in loop tick:");
            console.error(e);
            this.stop();
          }
        }
      });
      this.renderLoop = new timer.RenderLoop({
        tick: function(timerData){
          try {
            var now = +new Date;
            self.emit("render");
            ...
            console.warn("Exception in render loop tick:");
            console.error(e);
            this.stop();
          }
        }
    ...
    get: function(name){
      // look for a getter for this attribute
      var getter = this['_get'+name[0].toUpperCase() + name.substring(1) + 'Attr'];
      if(getter) {
        return getter.apply(this, Array.prototype.slice.call(arguments, 1));
      } else {
