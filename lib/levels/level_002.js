define([
  ...
  function createDoor() {
    var door = new Entity(
      {
        components: [collidable.name, portal.name, rect.name],
  ...
  function createPlayer () { 
    ...
    var player = Entity.get('thePlayer') || new Entity(
      {
        id: 'thePlayer',
        components: [keyboardControls.name, collidable.name, moveable.name, playerSprite.name, walking.name], 
  ...
  function createWallEntity(options){
    var wall = new Entity(
      {
        components: [collidable.name, rect.name], 
        name: "wall", 
